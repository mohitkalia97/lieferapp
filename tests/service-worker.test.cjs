...
test('install precaches the versioned optimizer and bypasses stale HTTP assets',async()=>{
  ...
  assert.ok(env.added.some(request=>request.url.endsWith('optimizer-ui.js?v=5.6')));
  assert.ok(env.added.some(request=>request.url.endsWith('routing.js?v=5.6')));
...
test('script fetches revalidate instead of silently reusing stale HTTP scripts',async()=>{
  ...
  const response=await env.dispatch('fetch',{request:new Request(origin+'/lieferapp/app.js?v=5.6')});
...
test('offline fetches serve only the matching version of a cached script',async()=>{
  ...
  env.stores.set('lieferroute-shell-v5-6',new Map([[origin+'/lieferapp/app.js?v=5.6',new Response('current cached script')]]));
  const response=await env.dispatch('fetch',{request:new Request(origin+'/lieferapp/app.js?v=5.6')});
  assert.equal(await response.text(),'current cached script');
  await assert.rejects(env.dispatch('fetch',{request:new Request(origin+'/lieferapp/app.js?v=5.7')}),/offline/);
});
test('a deployment error cannot overwrite a working cached optimizer',async()=>{
  ...
  env.stores.set('lieferroute-shell-v5-6',new Map([[origin+'/lieferapp/optimizer-ui.js?v=5.6',new Response('working optimizer')]]));
  const response=await env.dispatch('fetch',{request:new Request(origin+'/lieferapp/optimizer-ui.js?v=5.6')});
...
test('activation removes old app shells without clearing unrelated caches',async()=>{
  const env=environment();
  for(const key of ['lieferroute-shell-v5-5','lieferroute-shell-v5-6','another-project'])env.stores.set(key,new Map());
  await env.dispatch('activate');assert.deepEqual(env.removed,['lieferroute-shell-v5-5']);
});
